...
Miner and hauler range and Unstick impossible builds are two passes in the same file, run from ApplySettings (every load, and whenever an option signals Reapply) and from the Upkeep cue every ten minutes. Upkeep is a top-level instantiating cue with a check interval, so a savegame carries exactly one of it however often it is loaded.

The range is the maxbuy and maxsell parameters of the default MiningRoutine or TradeRoutine order of each Xenon ship found with primarypurpose mine - the job definitions give 0 to 2, and energy haulers use the same hull and order family. They are changed with edit_order_param, guarded by isparameditable, which restarts an order that is merely started and fails harmlessly on one in a critical state. Parameter values cannot be read back from MD, and MD cannot set a variable on a ship either - in play that failed with "Failed to set component.$DrJeleXenonTradeRange" - so the range last applied is kept in the table global.$DrJeleXenonTradeRangeApplied, keyed by ship. An entry is kept while its ship exists, not only while a pass finds it - in play find_ship returned about 250 of some 420 Xenon miners on each pass, and pruning the ones it missed re-edited 7 to 28 ships a pass, restarting their orders: a ship is edited once per change, a new job ship with the vanilla range is picked up by the
next pass, and while the setting is the vanilla 2 a ship that was never touched is left alone.

A build is impossible when neededbuildresources of the station's build storage lists a ware outside $BuildableWares - ore, silicon and energy cells, all Xenon station modules consume. Such a build is first aborted with abort_build container=build storage, exactly as factionlogic_economy cancels the queued builds of a station it deconstructs. A build already under way is not abortable - in play both Xenon expansions that needed a strut of another race failed to abort - and vanilla never uses remove_build on one, so it is instead given exactly the missing foreign wares with add_cargo on the build storage, and the next build step takes them like any delivery. The debug line says which of the two happened.
